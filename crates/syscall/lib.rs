...
pub fn sendto(fd: i32, buf: *const u8, len: usize, flags: i32, dest: *const u8, addrlen: usize) -> isize {
    unsafe { syscall6(44, fd as u64, buf as u64, len as u64, flags as u64, dest as u64, addrlen as u64) as isize }
}

pub fn recvfrom(fd: i32, buf: *mut u8, len: usize, flags: i32, src: *mut u8, addrlen: *mut usize) -> isize {
    unsafe { syscall6(45, fd as u64, buf as u64, len as u64, flags as u64, src as u64, addrlen as u64) as isize }
}
...
pub fn spawn_thread(f: extern "C" fn(*mut u8), arg: *mut u8, stack_size: usize) -> Result<(), ()> {
    ...
    if stack.is_null() { return Err(()); }
    
    let stack_top = unsafe { stack.add(stack_size) };
    const FLAGS: u64 = 0x100 | 0x200 | 0x400 | 0x800 | 17; // VM|FS|FILES|SIGHAND|SIGCHLD
    
    let ret = clone(FLAGS, stack_top, core::ptr::null_mut(), core::ptr::null_mut(), 0);
    if ret == 0 {
        f(arg);
        exit(0);
    }
